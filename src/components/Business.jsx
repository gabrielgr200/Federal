...
const FeatureCard = ({ icon, title, content, index }) => (
  <motion.div initial='hidden' animate='visible' className={`flex flex-row p-6 rounded-[20px] ${index !== features.length - 1 ? "mb-6" : "mb-0"} feature-card`}>
    ...
    <div className="flex-1 flex flex-col ml-3">
      <motion.h4 variants={slideInFromRight(0.5)} className="font-poppins font-semibold text-white text-[18px] leading-[23.4px] mb-1">
        {title}
      </motion.h4>
      <motion.p variants={slideInFromRight(0.8)} className="font-poppins font-normal text-dimWhite text-[16px] leading-[24px]">
        {content}
      </motion.p>
    </div>
  </motion.div>
...
const Business = () => (
  <motion.section initial='hidden' animate='visible' className={layout.section}>
    <div className={layout.sectionInfo}>
      <motion.h2 variants={slideInFromLeft(0.3)} className={styles.heading2}>
        O adiantamento de recebíveis via factoring é o modelo mais eficiente e mais seguro
        para empresas de todo o Brasil.
      </motion.h2>
      <motion.p variants={slideInFromLeft(0.5)} className={`${styles.paragraph} max-w-[470px] mt-5`}>
